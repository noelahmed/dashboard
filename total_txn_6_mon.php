...
$sql2 = "select count(*) TXN,sum(a.numamount) AMT,max(a.datauthorization) Last_succesful_transaction
	 from fcdbadminebl.admintxnunauthdata a where a.txnid in ('BPA','OAT','ITG','BDT')
	  and to_char(a.datauthorization,'yyyy-mm-dd')='$currentDateTime1'
and codstatus='5'
...
and  YEAR(processed_time )=$currentDateTime
	...
	$query2 = mysqli_query($connect,"
		...
		from eblconnect_lot_info_detail where processed_success='Y'
 and DATE(processed_time )='$currentDateTime1'
...
and topup_status='S'
and  to_char(trn_dt,'yyyy-mm-dd')='$currentDateTime1'
";
...
<?php }	
  ...
  to_char(sms_time,'yyyy-mm-dd')='$currentDateTime'
...
AND DATE(upload_time )='$currentDateTime'
...
$sql = "select count(*) MobileNo,CONVERT(varchar(24),max(callstart_datetime)) MobileNo1 from
 [avp_outbound].[dbo].[tbl_records]
  where   CAST(callstart_datetime AS date) = 
(select max(CAST(callstart_datetime AS date) ) 
